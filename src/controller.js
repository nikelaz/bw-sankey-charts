...
export default class SankeyController extends DatasetController {
  ...
  _drawLabels() {
    ...
    for (const node of nodes.values()) {
      ...
      if (x < chartArea.width / 2) {
        ctx.textAlign = 'left';
        textX += nodeWidth + borderWidth + 20;
      } else {
        ctx.textAlign = 'right';
  ...
  _drawLabel(label, y, height, ctx, textX, isSmaller) {
    ...
    if (isSmaller) {
      font.size = 13;
      font.lineHeight= 13;
      font.string = `normal ${font.weight} ${font.size}px ${font.family}`
    }
    ...
    const padding = valueOrDefault(this.options.padding, 50);
  ...
  _drawNodes() {
    ...
    for (const node of nodes.values()) {
      ...
      ctx.roundRect(x - 1, y, nodeWidth + 2, height, [40]);
    }
    ctx.restore();
  }
